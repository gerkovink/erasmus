...
Logistic regression performs better in every way than KNN. Remember that we started with a random seed. Every procedure that uses random numbers thereafter has become seed dependent. This also holds for the `train`/`test` split that we realized. A different random split can therefore yield different results. Crossvalidation - which we excluded in this practical - can give you an indication of the variance of this difference. 
